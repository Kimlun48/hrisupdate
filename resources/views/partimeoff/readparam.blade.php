<div class="border-body">
  <div class="row">
    <div class="col-md-9 justify-content-start">
      <button class="btn btn-sm btn-add" onclick="createpartimeoff()"><a  class="text-decoration-none text-light" >Create Time Off</a></button>
    </div>
    ...
    <table class="table data-table" id="myTable">
      <thead class="table-head">
        <tr class="judul">
          ...
          <th scope="col" onclick="sortTable(3)">Duration</th>
          <th scope="col" onclick="sortTable(4)">Code</th>
          <th scope="col" onclick="sortTable(5)">Effectife Date</th>
          <th scope="col" onclick="sortTable(6)">Expire Date</th>
          <th scope="col" onclick="sortTable(7)">Status</th>
          <th scope="col" onclick="sortTable(8)">Dokumen</th>
          <th scope="col" onclick="sortTable(9)">Kuota Cuti</th>
          <th scope="col" onclick="sortTable(10)"colspan="2">action</th>
        </tr>
      </thead>
      <tbody>      
        @foreach($param as $index => $item)
          <tr class="isi">
            <td class="name">{!! $param->firstItem() + $index !!}.</td>
            <td class="name">{{$item->type}}</td>
            <td class="name">{{$item->nama}}</td>
            <td class="name">{{$item->durasi}}</td>
            <td class="name">{{$item->kode}}</td>
            <td class="name">{{$item->efektif_date}}</td>
            <td class="name">{{$item->expire_date}}</td>
            <td class="status">
              <div class="bg_status" data-status="{{ $item->status }}">
                {{ $item->status }}
              </div>
            </td>
            <td class="name">{{$item->dokumen}}</td>
            <td class="name">{{$item->kuota}}</td>
