...
$(document).ready(function () {
    // Ambil nilai yang sudah tersimpan (jika ada) dari localStorage
    var selectedValue = localStorage.getItem('selectedValue');

    // Set nilai default dropdown
    if (selectedValue) {
        $('#showRows').val(selectedValue);
    }

    // Handle perubahan nilai dropdown
    $('#showRows').change(function () {
        var selectedValue = $(this).val();
        localStorage.setItem('selectedValue', selectedValue); // Simpan nilai ke localStorage

        $.get("{{ route('resignterm.readalmostexpired') }}", { show: selectedValue }, function (data) {
