<div class="border-body">
    <div class="row">
        <div class="col-md-9 justify-content-start">
            <button class="btn btn-sm btn-add" onClick="create()"><a  class="text-decoration-none text-light" >Bulk Data Cuti</a></button>
        </div>
        ...
        <table class="table data-table " id="myTable">
            ...
            @foreach ($cuti as $ct)
                ...
                <tr class="isi">
                    ...
                    <td class="name" >{{$ct->karyawan->nama_lengkap}}</td>
                    <td class="nik" >{{$ct->karyawan->nomor_induk_karyawan}}</td>
                    <td class="jml" >{{$ct->jumlah_cuti}}</td>
                    <td class="sisa" >{{$ct->sisa_cuti}}</td>
                    <td class="mulai" >{{$ct->mulai_cuti}}</td>
                    <td class="akhir" >{{$ct->akhir_cuti}}</td>            
                    <td class="tahun" >{{$ct->tahun}}</td>            
                    <td class="actions">
                        <div class="bg-edit" onClick="edit({{$ct->id}})">
                        ...
                        <!-- <button class="btn btn-sm btn-primary mb-2" onClick="edit({{$ct->id}})">
