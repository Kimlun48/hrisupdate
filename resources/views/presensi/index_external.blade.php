  ...
  <div class="container">
    <div class="row">
      <div class="head-title col-md-6">
        <h5 class="title-employ">Attendance External</h5>
      </div>
    ...
    <div class="col-md-5 dropdown drop-cztmng" style="">
      ...
      <div class="dropdown-menu dropcrl" id="employmentstatusSubMenu">
        ...
        @foreach ($employes as $status)
          <label class="dropdown-item" for="{{ $status->status_karyawan }}">
          <input class="statusCheckbox" type="checkbox" id="{{ $status->status_karyawan }}" value="{{ $status->status_karyawan }}">
...
@include('presensi.partials.scripts_external')
...
<script>
  function exportdata() {
    $.get("{{ url('/presensi/exportmodal_external') }}",{}, 
    function(data,status){
      $("#exportlabel").html('Export Presensi Data External');
