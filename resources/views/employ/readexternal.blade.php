<div class="tableling" id="employee-content">
  <table id="myTable" class="table data-table uppercase-text table-sort text-nowrap table-responsive">
    <thead class="table-head">
      <tr class="judul">
        ...
        <th scope="col" class="Branch col3" onclick="sortTable(3)">Type Employee <span class="sort-icon" id="sortIcon3"></span></th>
        <th scope="col" class="Brand col4" onclick="sortTable(4)">Brand <span class="sort-icon" id="sortIcon4"></span></th>
        <th scope="col" class="vendor col5" onclick="sortTable(5)">Vendor <span class="sort-icon" id="sortIcon5"></span></th>
        <th scope="col" class="Organization col6" onclick="sortTable(6)">Organization <span class="sort-icon" id="sortIcon6"></span></th>
        <th scope="col" class="Job-Position col7" onclick="sortTable(7)">Job Position <span class="sort-icon" id="sortIcon7"></span></th>
        <th scope="col" class="Job-Level col8" onclick="sortTable(8)">Job Level <span class="sort-icon" id="sortIcon8"></span></th>
        <th scope="col" class="Employment-Status col9" onclick="sortTable(9)">Employment Status <span class="sort-icon" id="sortIcon9"></span></th>
        <th scope="col" class="Join-Date col10" onclick="sortTable(10)">Join Date <span class="sort-icon" id="sortIcon10"></span></th>
        <th scope="col" class="End-Date col11" onclick="sortTable(11)">End Date <span class="sort-icon" id="sortIcon11"></span></th>
        <th scope="col" class="Sign-Date col12" onclick="sortTable(12)">Sign Date <span class="sort-icon" id="sortIcon12"></span></th>
        <th scope="col" class="Resign-Date col13" onclick="sortTable(13)">Resign Date <span class="sort-icon" id="sortIcon13"></span></th>
        <th scope="col" class="Barcode col14" onclick="sortTable(14)">Barcode <span class="sort-icon" id="sortIcon14"></span></th>
        <th scope="col" class="Email col15" onclick="sortTable(15)">Email <span class="sort-icon" id="sortIcon15"></span></th>
        <th scope="col" class="Birth-Date col16" onclick="sortTable(16)">Birth Date <span class="sort-icon" id="sortIcon16"></span></th>
        <th scope="col" class="Birth-Place col17" onclick="sortTable(17)">Birth Place <span class="sort-icon" id="sortIcon17"></span></th>
        <th scope="col" class="Address col18" onclick="sortTable(18)">Address <span class="sort-icon" id="sortIcon18"></span></th>
        <th scope="col" class="Mobile-Phone col19" onclick="sortTable(19)">Mobile Phone <span class="sort-icon" id="sortIcon19"></span></th>
        <th scope="col" class="Relegion col20" onclick="sortTable(20)">Relegion <span class="sort-icon" id="sortIcon20"></span></th>
        <th scope="col" class="Gender col21" onclick="sortTable(21)">Gender <span class="sort-icon" id="sortIcon21"></span></th>
        <th scope="col" class="Marital-Status col22" onclick="sortTable(22)">Marital Status <span class="sort-icon" id="sortIcon22"></span></th>
    ...
    <tbody class="table-body">
      ...
      <tr class="isi">
        ...
        <td class="cabang col3">{{ $employ->getjeniskar->nama}}</td>                            
        <td class="brand col4" id="brand" data-brand="{{ $employ->brand }}">{{ $employ->brand}}</td>
        <td class="vendor col5" id="vendor" data-vendor="{{ $employ->vendor }}">{{ $employ->vendor}}</td>
        <td class="bagian col6" id="bagian" data-bagian="{{ $employ->bagian->nama }}">{{ $employ->bagian->nama}}</td>
        <td class="jabatan col7" id="jabatan" data-jabatan="{{ $employ->jabatan->nama }}">{{ $employ->jabatan->nama}}</td>
        <td class="level col8" id="level" data-level="$employ->jabatan->paramlevel->nama">{{$employ->jabatan->paramlevel->nama}}</td>
        <td class="posisi col9" id="posisi" data-posisi="{{ $employ->status_karyawan }}">{{ $employ->status_karyawan}}</td>
        <td class="tahun_gabung col10" id="tahun_gabung">{{ date('d F Y', strtotime($employ->tahun_gabung)) }}</td>
        <td class="expired_kontrak col11" id="expired_kontrak">{{date('d F Y', strtotime($employ->expired_kontrak)) }}</td>
        <td class="sign col12" id="sign"></td>
        <td class="tahun_keluar col13" id="tahun_keluar">{{ date('d F Y', strtotime($employ->tahun_keluar)) }}</td>
        <td class="no_finger col14" id="no_finger">{{ $employ->no_finger}}</td>
        <td class="email col15" id="email">{{ $employ->email}}</td>
        <td class="tgl_lahir col16" id="tgl_lahir">{{ date('d F Y', strtotime($employ->tgl_lahir)) }}</td>
        <td class="tempat_lahir col17" id="tempat_lahir">{{ $employ->tempat_lahir}}</td>
        <td class="alamat col18" id="alamat">{{ $employ->alamat}}</td>
        <td class="no_hp col19" id="no_hp">{{ $employ->no_hp}}</td>
        <td class="agama col20" id="agama">{{ $employ->agama}}</td>
        <td class="gender col21" id="gender">{{ $employ->gender}}</td>
        <td class="status_pernikahan col22" id="status_pernikahan">{{ $employ->status_pernikahan}}</td>
      ...
      @empty
      <tr>
        <td colspan="8" class="text-center alert alert-danger">Data belum tersedia.</td>
      </tr>
      @endforelse
    </tbody>
  </table>
...
<script>
  $(document).ready(function () {
    ...
    function loadPage(page) {
      ...
      $.ajax({
        url: '/employ/index_read_external?page=' + page,
        type: 'GET',
        data: {
          status: statusAktif,
