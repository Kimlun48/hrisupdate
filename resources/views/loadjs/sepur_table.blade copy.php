...
var sortDirections = ["asc", "asc"];
var sortIcons = document.getElementsByClassName("sort-icon");
var sortColumnIndex = 0; // Indeks kolom awal untuk sorting (misalnya kolom pertama)
...
function sortTable(columnIndex) {
  ...
  rows = Array.from(table.rows).slice(1); // Convert HTMLCollection to Array and exclude the header row
  ...
  // Reset current page to 1 and update the rows and pagination
  currentPage = 1;
...
function handlePaginationClick(event) {
  ...
  // Update current page and show rows for the clicked page
  currentPage = clickedPage;
  showRows();
  renderPagination();

  // Re-apply sorting based on the current sort column
  sortTable(sortColumnIndex);
}

// Call the handlePaginationClick function after each page change
