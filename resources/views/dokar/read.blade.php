<div class="border-body">
    <div class="row">
        <div class="col-md-9 justify-content-start">
            <button class="btn btn-sm btn-add" onClick="create()"><a  class="text-decoration-none text-light" >Add Document Employee</a></button>
        </div>
        ...
        <table class="table data-table" id="myTable">
            ...
            @forelse ($dok as $par)
                <tr class="isi">
                    ...
                    <td class="name" >{{$par->getkar->nama_lengkap}}</td>
                    <td class="name">{{ $par->getkar->nomor_induk_karyawan }}</td>
                    <td class="name">{{ $par->nama }}</td>
                    <td class="name">{{ $par->tipe_dok }}</td>
                    <td class="name">{{ $par->nomor_dok }}</td>
                    <td class="name">{{ $par->lokasi_penyimpanan}}</td>
                    <!-- <td class="name"><img src="/storage/DokumenKaryawan/{{$par->dok_file}}" style="border-radius:15px 50px;height:35px;width: 35%;"><br><a href="/storage/DokumenKaryawan/{{$par->dok_file}}" download> {{ $par->dok_file }}</a></td> -->
                    <td class="name">
                        <a href="http://10.1.0.9/staging_hris/DokKar/{{$par->dok_file}}" target="_blank">
                            DOK-{{ $par->tipe_dok }}-{{$par->getkar->nama_lengkap}}
                        </a>
                    </td>
