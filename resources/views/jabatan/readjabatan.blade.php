<div class="border-body">
    <div class="row">
        <div class="col-md-9 justify-content-start">
            <button class="btn btn-sm btn-add" onClick="create()"><a  class="text-decoration-none text-light" >Add Postion Parent</a></button>
        </div>
        ...
        <table class="table data-table" id="myTable">
            ...
            @forelse ($jabs as $jab)                    
                <tr class="isi">
                    ...
                    <td class="name">{{ $jab->nama }}</td>
                    <td class="kode">
                        @if($jab->kode == 1)
                            <div class="status-approve">APPROVE</div>
                        @else
                            <div class="status-nonapprove">NON-APPROVE</div>
                        @endif
                    </td>
                    <td class="status text-uppercase">
                        <div class="bg_status" data-status="{{ $jab->status }}">
                            @if($jab->status == "aktif")
                    ...
                    @if($jab->parent_id)
                        <td class="position">{{ $jab->parent->nama }}</td>
                    @else
                        <td></td>
                    @endif
                    <td class="actions">
                        <div class="bg-detail" onClick="showisi({{$jab->id}})">
                        ...
                        <div class="bg-edit" onClick="edit({{$jab->id}})">
                        ...
                        <!-- <button class="btn btn-sm btn-info mb-2" onClick="showisi({{$jab->id}})">
                            <i class="fas fa-eye" style="color:#fff; width:20px"></i>
                        </button> 
                        <button class="btn btn-sm btn-primary mb-2" onClick="edit({{$jab->id}})">
                            <i class="fas fa-pencil-alt" style="color:#fff; width:20px"></i>
                        </button>  -->
                    </td>
                </tr>
                @empty                   
...
@include('jabatan.partials.script')
