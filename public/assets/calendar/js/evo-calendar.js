...
;(function(factory) {
    ...
    EvoCalendar.prototype.buildTheBones = function() {
        ...
        markup += '<div class="event-header"><p></p></div>';
        markup += '<ul class="nav nav-tabs" id="eventTabs">';
        markup += '<li class="nav-item">';
        markup += '<a class="nav-link active" id="event-tab-1" data-toggle="tab" href="#event-1">All Events</a>';
        markup += '</li>';
        markup += '<li class="nav-item">';
        markup += '<a class="nav-link" id="event-tab-2" data-toggle="tab" href="#event-2">Tab 2</a>';
        markup += '</li>';
        markup += '</ul>';
        markup += '<div class="tab-content">';
        markup += '<div class="tab-pane fade show active" id="event-1"><div class="event-list"></div></div>';
        markup += '<div class="tab-pane fade" id="event-2">Konten Tab 2</div>';
        markup += '</div>';
        markup += '</div>';
    ...
    EvoCalendar.prototype.buildEventList = function() {
        var _ = this, markup, hasEventThisMonth = false;
        ...
        if (_.options.calendarEvents) {
            // Sorting the events by date (day/month) in ascending order
            _.options.calendarEvents.sort(function(a, b) {
                var dateA = new Date(a.date);
                var dateB = new Date(b.date);
                var dayMonthA = (dateA.getMonth() + 1) + '/' + dateA.getDate(); // Format d/m
                var dayMonthB = (dateB.getMonth() + 1) + '/' + dateB.getDate(); // Format d/m
                return dayMonthA.localeCompare(dayMonthB);
            });
            
            for (var i = 0; i < _.options.calendarEvents.length; i++) {
                var eventDate = new Date(_.options.calendarEvents[i].date);
                var currentMonth = _.$active.month; // Menggunakan bulan yang dipilih dari $active
                var eventMonth = eventDate.getMonth();
                
                if (currentMonth === eventMonth) {
                    eventAdder(_.options.calendarEvents[i]);
                    hasEventThisMonth = true;
                }
            }
        }
        ...
        // IF: no event for the selected month
        if (!hasEventThisMonth) {
            markup = '<div class="event-empty">';
            markup += '<p>'+_.initials.dates[_.options.language].noEventForThisMonth+'</p>';
            markup += '</div>';
        }
